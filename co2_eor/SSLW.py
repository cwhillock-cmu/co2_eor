...
class SSLWCostingData(FlowsheetCostingBlockData):
    ...
    def cost_vessel(
        ...
        if vessel_thickness is None:
            try:
                vessel_thickness = blk.unit_model.thickness
            except AttributeError:
                raise ConfigurationError(
                    f"{blk.unit_model.name} does not have a component named "
                    "thickness. Please provide a reference to the vessel "
                    "thickness as an argument to cost_unit."
                )
        ...
        t_in = pyo.units.convert(vessel_thickness, to_units=pyo.units.inch)
