...
#m.fs.pipe5 = liqPipe(**liqPipe_config,length=20400)
...
#m.fs.pipe11 = gasPipe(**gasPipe_config,length=21200)
...
m.fs.prod_target = pyo.Param(mutable=True,initialize=200000) #STB/yr
...
m.fs.obj_no_revenue.deactivate()
...
length_vals = np.linspace(2400,20400,51)
...
for length in length_vals:
    run_conopt = False
    m.fs.pipe5.length = length
    m.fs.pipe11.length = length+800
    print(f'starting solve pipe5 length = {length}')
    #"""
    try:
        scaled_m = pyo.TransformationFactory("core.scale_model").create_using(m)
        with open(rf'temps/bakken_length_sweep/bakken_{int(length)}_ipopt_output.txt', 'w') as f:
            with contextlib.redirect_stdout(f):
                res = ipopt.solve(scaled_m,tee=True)
        if res.solver.termination_condition == pyo.TerminationCondition.optimal:
            pyo.TransformationFactory("core.scale_model").propagate_solution(scaled_m,m)
            with open(rf'temps/bakken_length_sweep/bakken_{int(length)}_postsolve_display.txt', 'w') as f:
                with contextlib.redirect_stdout(f):
                    m.display()
            print('ipopt converged')
            print(f'length {length} solved')
            export_flowsheet_to_excel(m.fs, rf'temps/bakken_length_sweep/bakken_{int(length)}_export.xlsx')
        else:
            run_conopt = True
    ...
    if run_conopt:
        ...
        try:
            scaled_m = pyo.TransformationFactory("core.scale_model").create_using(m)
            with open(rf'temps/bakken_length_sweep/bakken_{int(length)}_conopt_output.txt', 'w') as f:
                with contextlib.redirect_stdout(f):
            ...
            if res.solver.termination_condition == pyo.TerminationCondition.optimal:
                ...
                print(f'conopt {length} not optimal')
        except ValueError:
            print(f'conopt failed')
            pass
