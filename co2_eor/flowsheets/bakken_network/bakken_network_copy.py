...
prod_targets = np.flip(np.arange(350000,475000,25000))
ipopt.options['max_iter'] = 150
ipopt.options['linear_solver']='ma27'
ipopt.options['acceptable_tol']=1E-6
ipopt.options['tol']=1E-8
...
for prod_target in prod_targets:
    ...
    #"""
    try:
        with open(rf'temps/bakken_sweep/bakken_{int(prod_target)}_ipopt_output.txt', 'w') as f:
            with contextlib.redirect_stdout(f):
                res = ipopt.solve(scaled_m,tee=True)
    ...
    if run_conopt:
        ...
        with open(rf'temps/bakken_sweep/bakken_{int(prod_target)}_conopt_output.txt', 'w') as f:
            with contextlib.redirect_stdout(f):
