...
def define_state(b):
    ...
    expected_keys = ["flow_mass", "temperature", "pressure"]
    if (
        ...
        for k in b.params.config.state_bounds.keys():
            if "mass_frac" in k:
                _log.warning(
                    ...
                    " Mass fraction bounds are set automatically and "
    ...
    #def c_mw(b,i):
    #    return b.params.get_component(i).mw
    ...
    #b.avg_mw = Expression(expr=sum(b.mole_frac_comp[i]*b.mw_comp[i] for i in b.component_list))
    ...
    b.flow_mol = Expression(expr=b.flow_mass*b.mw)
    ...
    # -------------------------------------------------------------------------
    # General Methods
    def get_material_flow_terms_mFTPx(b, p, j):
        """Create material flow terms for control volume."""
        #return b.flow_mol_phase_comp[p, j]
        return b.flow_mass_phase_comp[p,j]

    b.get_material_flow_terms = MethodType(get_material_flow_terms_mFTPx, b)

    def get_enthalpy_flow_terms_mFTPx(b, p):
        ...
        try:
            ...
            def rule_eflow(b, p):
                ...
                return b.flow_mass_phase[p] * b.enth_mol_phase[p] * b.mw_phase[p]

            eflow = b._enthalpy_flow_term = Expression(b.phase_list, rule=rule_eflow)
        return eflow[p]

    b.get_enthalpy_flow_terms = MethodType(get_enthalpy_flow_terms_mFTPx, b)

    def get_material_density_terms_mFTPx(b, p, j):
    ...
    b.get_material_density_terms = MethodType(get_material_density_terms_mFTPx, b)

    def get_energy_density_terms_mFTPx(b, p):
    ...
    b.get_energy_density_terms = MethodType(get_energy_density_terms_mFTPx, b)
    ...
    b.define_state_vars = MethodType(define_state_vars_mFTPx, b)
    ...
    b.define_display_vars = MethodType(define_display_vars_mFTPx, b)
    ...
    for p in b.phase_list:
        ...
        for j in b.component_list:
            if (p, j) in b.phase_component_set:
                ...
                b.mole_frac_phase_comp[p, j].value = value(b.mole_frac_comp[j])
...
def define_default_scaling_factors(b):
    ...
    #b.set_default_scaling("flow_mol_comp", 1 / f_init) DNE
...
def calculate_scaling_factors(b):
    ...
    if b.config.defined_state is False:
        iscale.constraint_scaling_transform(
            b.sum_mass_frac_out, min(sf_mf.values()), overwrite=False
        )
