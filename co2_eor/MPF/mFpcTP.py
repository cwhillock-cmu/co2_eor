...
modified to mass basis for co2_eor
removed electrolyte stuff
"""
...
from co2_eor.MPF.mFTPx import mFTPxScaler
...
def define_state(b):
    ...
    expected_keys = ["flow_mass_phase_comp", "enth_mass", "temperature", "pressure"]
    if (
    ...
    f_bounds, f_init = get_bounds_from_config(b, "flow_mass_phase_comp", units.FLOW_MASS)
    ...
    # Add state variables
    b.flow_mass_phase_comp = Var(
        b.phase_component_set,
        initialize=f_init,
        domain=NonNegativeReals,
        bounds=f_bounds,
        doc="Phase-component mass flowrate",
    ...
    @b.Expression(b.phase_list)
    def flow_mass_phase(b,p):
        return sum(
    ...
    b.flow_mass = Expression(
        expr=sum(b.flow_mass_phase_comp[i] for i in b.phase_component_set),
        doc="Total mass flowrate",
    )

    b.mass_frac_phase_comp = Var(
        b.phase_component_set,
        bounds=(1e-20, 1.001),
        initialize=1 / len(b.component_list),
        doc="Phase mass fractions",
        units=pyunits.dimensionless,
    )

    @b.Constraint(b.phase_component_set)
    def mole_frac_phase_comp_eq(b,p,j): #probably shouldn't change name of this constraint
        # Calculating mass frac phase comp is degenerate if there is only one
        # component in phase.
        # Count components
        comp_count = 0
    ...
    def mole_frac_phase_comp(b,p,j):
        #need to get which components are in the phase
        denominator = 0
        for p1, c1 in b.phase_component_set:
            if p1 == p:
                denominator += b.mass_frac_phase_comp[p1,c1]/b.mw_comp[c1]
    
        return (b.mass_frac_phase_comp[p,j]/b.mw_comp[j])/denominator

    b.mass_frac_comp = Var(
        b.component_list,
        bounds=(1e-20, 1.001),
        initialize=1 / len(b.component_list),
        doc="Mixture mass fractions",
        units=pyunits.dimensionless,
    )

    @b.Constraint(b.component_list)
    def mole_frac_comp_eq(b,i): #cant change name of this constraint 
        return b.flow_mass_comp[i] == b.mass_frac_comp[i] * sum(b.mass_frac_comp[k] for k in b.component_list)
    
    @b.Expression(b.component_list)
    def mole_frac_comp(b,j):
        return (b.mass_frac_comp[j]/b.mw_comp[j])/sum(b.mass_frac_comp[k]/b.mw_comp[k] for k in b.component_list)
    
    @b.Expression(b.phase_component_set)
    def flow_mol_phase_comp(b,p,j):
        return b.flow_mass_phase_comp[p,j] / b.mw_comp[j]
    ...
    @b.Expression(b.component_list)
    def flow_mol_comp(b,j):
        return b.flow_mass_comp[j] / b.mw_comp[j]
    
    @b.Expression()
    def flow_mol(b):
        return sum(b.flow_mol_comp[k] for k in b.component_list)
    ...
    def rule_phase_frac(b, p):
        if len(b.phase_list) == 1:
            ...
            return b.flow_mass_phase[p] / b.flow_mass

    b.phase_frac = Expression(b.phase_list, rule=rule_phase_frac, doc="Phase fractions")

    # -------------------------------------------------------------------------
    # General Methods
    def get_material_flow_terms_mFpcTP(b, p, j):
        """Create material flow terms for control volume."""
        return b.flow_mass_phase_comp[p, j]

    b.get_material_flow_terms = MethodType(get_material_flow_terms_mFpcTP, b)

    def get_enthalpy_flow_terms_mFpcTP(b, p):
        ...
        try:
            ...
            def rule_eflow(b, p):
                return b.flow_mol_phase[p] * b.enth_mol_phase[p] * b.mw_phase[p]

            eflow = b._enthalpy_flow_term = Expression(b.phase_list, rule=rule_eflow)
        return eflow[p]

    b.get_enthalpy_flow_terms = MethodType(get_enthalpy_flow_terms_mFpcTP, b)

    def get_material_density_terms_mFpcTP(b, p, j):
        ...
        try:
            ...
            def rule_mdens(b, p, j):
                return b.dens_mass_phase[p] * b.mass_frac_phase_comp[p, j]
    ...
    b.get_material_density_terms = MethodType(get_material_density_terms_mFpcTP, b)

    def get_energy_density_terms_mFpcTP(b, p):
        ...
        try:
            ...
            def rule_edens(b, p):
                return b.dens_mass_phase[p] * b.energy_internal_mol_phase[p] * b.mw_phase[p]

            edens = b._energy_density_term = Expression(b.phase_list, rule=rule_edens)
        return edens[p]

    b.get_energy_density_terms = MethodType(get_energy_density_terms_mFpcTP, b)

    def default_material_balance_type_mFpcTP():
        return MaterialBalanceType.componentTotal

    b.default_material_balance_type = default_material_balance_type_mFpcTP

    def default_energy_balance_type_mFpcTP():
        return EnergyBalanceType.enthalpyTotal

    b.default_energy_balance_type = default_energy_balance_type_mFpcTP

    def get_material_flow_basis_mFpcTP():
        return MaterialFlowBasis.mass

    b.get_material_flow_basis = get_material_flow_basis_mFpcTP

    def define_state_vars_mFpcTP(b):
        """Define state vars."""
        return {
    ...
    b.define_state_vars = MethodType(define_state_vars_mFpcTP, b)

    def define_display_vars_mFpcTP(b):
        """Define display vars."""
        return {
    ...
    b.define_display_vars = MethodType(define_display_vars_mFpcTP, b)


def state_initialization(b):
    for i in b.phase_component_set:
        b.mass_frac_phase_comp[i].value = value(
            b.flow_mass_phase_comp[i] / b.flow_mass_phase[i[0]]
        )
...
def define_default_scaling_factors(b):
    ...
    try:
        f_bounds = state_bounds["flow_mass_phase_comp"]
        if len(f_bounds) == 4:
            f_init = pyunits.convert_value(
                f_bounds[1], from_units=f_bounds[3], to_units=units.FLOW_MASS
            )
        else:
            f_init = f_bounds[1]
    except KeyError:
    ...
    b.set_default_scaling("flow_mass", 1 / f_init)
    b.set_default_scaling("flow_mass_phase", 1 / f_init)
    b.set_default_scaling("flow_mass_comp", 1 / f_init)
    b.set_default_scaling("flow_mass_phase_comp", 1 / f_init)
...
def calculate_scaling_factors(b):
    for p, j in b.phase_component_set:
        sf = iscale.get_scaling_factor(
            b.flow_mass_phase_comp[p, j], default=1, warning=True
        )
        iscale.constraint_scaling_transform(
            b.mass_frac_phase_comp_eq[p, j], sf, overwrite=False
        )
...
class mFpcTPScaler(mFTPxScaler):
    """
    Scaler for mFpcTP state variables
    """

    # Inherit variable_scaling_routine from mFTPx.
