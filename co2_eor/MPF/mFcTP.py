...
converted to mass basis for co2 eor project
"""
...
def define_state(b):
    ...
    expected_keys = ["flow_mass_comp", "temperature", "pressure"]
    if (
    ...
    f_bounds, f_init = get_bounds_from_config(b, "flow_mass_comp", units.FLOW_MASS)
    ...
    @b.Expression()
    def flow_mass(b):
        return sum(b.flow_mass_comp[i] for i in b.component_list)
    ...
    def flow_mass_phase(b,p):
        return b.flow_mol_phase[p] * b.mw_phase[p]
    ...
    def mole_frac_comp(b,i):
        return (b.mass_frac_comp[i]/b.mw_comp[i])/sum(b.mass_frac_comp[j]/b.mw_comp[j] for j in b.component_list)
    ...
    def flow_mass_phase_comp(b,p,j):
        return b.flow_mol_phase_comp[p,j] * b.mw_comp[j]
    ...
    # -------------------------------------------------------------------------
    # General Methods
    def get_material_flow_terms_mFcTP(b, p, j):
        """Create material flow terms for control volume."""
        return b.flow_mass_phase_comp[p, j]

    b.get_material_flow_terms = MethodType(get_material_flow_terms_mFcTP, b)

    def get_enthalpy_flow_terms_mFcTP(b, p):
    ...
    b.get_enthalpy_flow_terms = MethodType(get_enthalpy_flow_terms_mFcTP, b)

    def get_material_density_terms_mFcTP(b, p, j):
    ...
    b.get_material_density_terms = MethodType(get_material_density_terms_mFcTP, b)

    def get_energy_density_terms_mFcTP(b, p):
    ...
    b.get_energy_density_terms = MethodType(get_energy_density_terms_mFcTP, b)

    def default_material_balance_type_mFcTP():
        return MaterialBalanceType.componentTotal

    b.default_material_balance_type = default_material_balance_type_mFcTP

    def default_energy_balance_type_mFcTP():
        return EnergyBalanceType.enthalpyTotal

    b.default_energy_balance_type = default_energy_balance_type_mFcTP

    def get_material_flow_basis_mFcTP():
        return MaterialFlowBasis.mass

    b.get_material_flow_basis = get_material_flow_basis_mFcTP

    def define_state_vars_mFcTP(b):
        """Define state vars."""
        return {
            "flow_mass_comp": b.flow_mass_comp,
            "temperature": b.temperature,
            "pressure": b.pressure,
        }

    b.define_state_vars = MethodType(define_state_vars_mFcTP, b)

    def define_display_vars_mFcTP(b):
        """Define display vars."""
        return {
            "Mass Flowrate": b.flow_mass_comp,
            "Temperature": b.temperature,
            "Pressure": b.pressure,
        }

    b.define_display_vars = MethodType(define_display_vars_mFcTP, b)
...
def define_default_scaling_factors(b):
    ...
    try:
        f_bounds = state_bounds["flow_mass_comp"]
        if len(f_bounds) == 4:
