...
def set_metadata(b):
    ...
    b.get_metadata().properties["enth_mass"].set_method(None)
...
def define_state(b):
    ...
    expected_keys = ["flow_mass", "enth_mass", "temperature", "pressure"]
    if (
    ...
    f_bounds, f_init = get_bounds_from_config(b, "flow_mass", units.FLOW_MASS)
    h_bounds, h_init = get_bounds_from_config(b, "enth_mass", units.ENERGY_MASS)
    ...
    def mole_frac_comp(b,i):
        return (b.mass_frac_comp[i]/b.mw[i])/sum(b.mass_frac_comp[j]/b.mw[j] for j in b.component_list)
    ...
    b.enth_mass = Var(
        initialize=h_init,
        bounds=h_bounds,
        doc="State mass enthalpy",
        units=units.ENERGY_MASS,
    )

    @b.Expression(b.component_list)
    def enth_mole(b,i):
        return b.enth_mass[i]/b.mw[i]
    ...
    b.mw = Expression(expr=sum(b.mole_frac_comp[i]*b.mw[i] for i in b.component_list))

    b.flow_mole = Expression(expr=b.flow_mass*b.mw)
    ...
    """
    b.flow_mass_phase = Var(
        b.phase_list,
        initialize=fp_init,
        domain=NonNegativeReals,
        bounds=f_bounds,
        doc="Phase mass flow rates",
        units=units.FLOW_MASS,
    )
    ...
    def flow_mass_phase_comp_rule(b, p, j):
        return b.flow_mass_phase[p] * b.mass_frac_phase_comp[p, j]

    b.flow_mass_phase_comp = Expression(
        b.phase_component_set, rule=flow_mass_phase_comp_rule
    )

    @b.Expression(b.phase_list)
    def phase_mw(b,p):
        return sum(b.mole_frac_phase_comp[p,i]*b.mw[i] for i in b.component_list)
    """
    ...
    def flow_mass_phase_comp(b,p,i):
        return b.flow_mole_phase_comp[p,i]*b.mw[i]
    
    @b.Expression(b.phase_component_set)
    def mass_frac_phase_comp(b,p,i):
        return b.mole_frac_phase_comp[p,i]*b.mw[i]/sum(b.mole_frac_phase_comp[p,j]*b.mw[j] for j in b.component_list)
    ...
    # -------------------------------------------------------------------------
    # General Methods
    def get_material_flow_terms_mFPhx(b, p, j):
        """Create material flow terms for control volume."""
        #return b.flow_mol_phase_comp[p, j]
        return b.flow_mass_phase_comp[p, j]

    b.get_material_flow_terms = MethodType(get_material_flow_terms_mFPhx, b)

    def get_enthalpy_flow_terms_mFPhx(b, p):
        ...
        try:
            ...
            def rule_eflow(b, p):
                ...
                return b.flow_mass_phase[p] * b.enth_mol_phase[p] * b.phase_mw[p]

            eflow = b._enthalpy_flow_term = Expression(b.phase_list, rule=rule_eflow)
        return eflow[p]

    b.get_enthalpy_flow_terms = MethodType(get_enthalpy_flow_terms_mFPhx, b)

    def get_material_density_terms_mFPhx(b, p, j):
    ...
    b.get_material_density_terms = MethodType(get_material_density_terms_mFPhx, b)

    def get_energy_density_terms_mFPhx(b, p):
        ...
        try:
            ...
            def rule_edens(b, p):
                return b.dens_mass_phase[p] * b.energy_internal_mol_phase[p] * b.phase_mw[p]

            edens = b._energy_density_term = Expression(b.phase_list, rule=rule_edens)
        return edens[p]

    b.get_energy_density_terms = MethodType(get_energy_density_terms_mFPhx, b)

    def default_material_balance_type_mFPhx():
        return MaterialBalanceType.componentTotal

    b.default_material_balance_type = default_material_balance_type_mFPhx

    def default_energy_balance_type_mFPhx():
        return EnergyBalanceType.enthalpyTotal

    b.default_energy_balance_type = default_energy_balance_type_mFPhx

    def get_material_flow_basis_mFPhx():
        return MaterialFlowBasis.mass

    b.get_material_flow_basis = get_material_flow_basis_mFPhx

    def define_state_vars_mFPhx(b):
        """Define state vars."""
        return {
            "flow_mass": b.flow_mass,
            "mass_frac_comp": b.mass_frac_comp,
            "enth_mass": b.enth_mass,
            "pressure": b.pressure,
        }

    b.define_state_vars = MethodType(define_state_vars_mFPhx, b)

    def define_display_vars_mFPhx(b):
        """Define display vars."""
        return {
            "Total Mass Flowrate": b.flow_mass,
            "Total Mass Fraction": b.mass_frac_comp,
            "Mass Enthalpy": b.enth_mass,
            "Pressure": b.pressure,
        }

    b.define_display_vars = MethodType(define_display_vars_mFPhx, b)
...
def define_default_scaling_factors(b):
    ...
    try:
        f_bounds = state_bounds["flow_mass"]
        if len(f_bounds) == 4:
    ...
    try:
        h_bounds = state_bounds["enth_mass"]
        if len(h_bounds) == 4:
            h_init = pyunits.convert_value(
                h_bounds[1], from_units=h_bounds[3], to_units=units.ENERGY_MASS
            )
        else:
            h_init = h_bounds[1]
    except KeyError:
    ...
    #b.set_default_scaling("flow_mol_comp", 1 / f_init) this does not exist
    ...
    b.set_default_scaling("enth_mass", 1 / h_init)
    ...
    def constraint_scaling_routine(
        ...
        if model.config.defined_state is False:
            self.set_component_scaling_factor(
                model.sum_mass_frac_out, 1, overwrite=overwrite
            )

        sf_enth = self.get_scaling_factor(model.enth_mass)
