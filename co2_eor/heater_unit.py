...
changes for co2_eor:
    added quick area calculation assuming cooling water cooler
    added power estimate for cooling water cooler
"""
...
def make_custom_costing(unit):
    from idaes.core.util.math import safe_log, smooth_abs
    #Q=UAdeltaTLM

    unit.U = pyo.Param(initialize=300,units=units.W/units.m**2/units.K) #from engineering toolbox
    #unit.area = pyo.Var(initialize=100,bounds=(0,None),units=units.m**2)

    #unit.T_CW_in = pyo.Var(initialize=302,units=units.K)
    #unit.T_CW_out = pyo.Var(initialize=309,units=units.K)
    #unit.T_CW_in.fix()
    #unit.T_CW_out.fix()

    #unit.dT1 = pyo.Expression(expr=unit.control_volume.properties_in[0].temperature-unit.T_CW_out)
    #unit.dT2 = pyo.Expression(expr=unit.control_volume.properties_out[0].temperature-unit.T_CW_in)

    #unit.HX_eq = pyo.Constraint(expr=
    #    #-unit.control_volume.heat[0]*(safe_log(unit.dT1)-safe_log(unit.dT2))==unit.U*unit.area*(unit.dT1-unit.dT2)
    #    -unit.control_volume.heat[0]==unit.U*unit.area*71.5 #simplify LMTD to 71.5
    #    )

    unit.LMTD = pyo.Var(initialize=71.5,units=units.K)
    unit.LMTD.fix()
    ...
    unit.q_CW = pyo.Expression(expr=
        -unit.control_volume.heat[0]/(4184*unit.CW_dT*1000) #(J/s) / (J/kg/K) / (K - K) / (kg/m^3) = m^3/s
        )
