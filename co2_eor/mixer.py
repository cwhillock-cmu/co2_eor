...
**MomentumMixingType.inequality** - enforces that mixed pressure is less than all inlet pressures}""",
        ),
    )
    CONFIG.declare(
        "mixed_state_block",
...
**False** - do not construct Ports.""",
    ...
    def build(self):
        ...
        try:
            if self.config.momentum_mixing_type == MomentumMixingType.minimize:
                ...
                pass
            elif self.config.momentum_mixing_type == MomentumMixingType.inequality:
                self.add_pressure_inequality_equations(
                        inlet_blocks=inlet_blocks, mixed_block=mixed_block
                        )
            else:
                raise ConfigurationError(
    ...
    def add_pressure_inequality_equations(self, inlet_blocks, mixed_block):
        """
        Add pressure inequality equations. Note that this writes a number of
        constraints equal to the number of inlets, enforcing mixed stream less
        than or equal to all inlet
        """
        if not hasattr(self, "inlet_idx"):
            self.inlet_idx = RangeSet(len(inlet_blocks))
        ...
        @self.Constraint(
            self.flowsheet().time,
            self.inlet_idx,
            doc="mixed pressure inequality constraint",
        )
        def pressure_inequality_constraints(b, t, i):
            return mixed_block[t].pressure <= inlet_blocks[i - 1][t].pressure
